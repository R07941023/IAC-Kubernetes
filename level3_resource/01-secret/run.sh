...
echo "[Start] namespace: logging"
kubectl create -n logging secret tls mydormroom-tls --cert=mydormroom.crt --key=mydormroom.key --save-config --dry-run=client -o yaml > temp-secret.yaml
kubectl apply -f temp-secret.yaml
rm -f temp-secret.yaml
echo "[End] namespace: logging"
